...
1. CAN和CANFD通信：主要聚焦 **数据链路层和物理层的设计**，就是怎么在总线上传输，如果解决碰撞问题。各节点监听总线，空闲时发送数据，数据竞争通过ID大小来看，ID越小优先级越大，同时CAN数据帧会有错误码校验。CANFD相比于CAN传输数据长度变大，通信效率更高。本质上还是基于信号通信，发现信号错了就重发。
2. 基于以太网的SOA：
